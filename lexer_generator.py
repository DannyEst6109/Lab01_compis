...
Recibe un DFA combinado (resultante de unir todas las posibles expresiones que delimita el yalex)
y escribe un archivo .py autónomo que implementa el analizador léxico.

Estructura esperada de `dfa_result` (autómata de input)
------------------------------------
{
    "initial_state": 0,
...
def _gen_actions(lines: list[str], dfa: dict) -> None:
    ...
    _w(lines)
    _w(lines, "def _run_action(token: str, lxm: str):")
    _w(lines, '    """')
    _w(lines, "    Ejecuta la acción asociada al token reconocido.")
    _w(lines, "    Retorna el valor producido por la acción, o None si debe ignorarse.")
    _w(lines, '    """')

    # Una rama por token distinto (puede haber varios estados con el mismo token)
    seen: set[str] = set()
    for info in dfa["accepting_states"].values():
        ...
        _w(lines, f"    if token == {token_name!r}:")
        if action:
    ...
    _w(lines, "    return None  # acción no reconocida → ignorar")
    _w(lines)
...
def _gen_lexer_function(lines: list[str], dfa: dict) -> None:
    ...
    _w(lines)
    _w(lines, "    Cada token es el valor retornado por su acción en el .yal.")
    _w(lines, "    Los tokens cuya acción retorna None son silenciosamente ignorados.")
    _w(lines)
    _w(lines, "    Lanza LexError si encuentra un carácter no reconocible.")
    _w(lines, '    """')
    ...
    _w(lines, "                # Actualiza solo si este patrón tiene mayor prioridad")
    ...
    _w(lines, "            raise LexError(")
    _w(lines, "                f'Error léxico en posición {pos}: carácter {buffer[pos]!r} no reconocido.'")
    _w(lines, "            )")
    _w(lines)
    _w(lines, "        lxm    = buffer[pos:last_accept_pos]")
    _w(lines, "        result = _run_action(last_accept_token, lxm)")
